...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['GROQ_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Groq) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new Groq({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'GROQ_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
